...
// step. Surfaced on the wire via pit-diag 0x6C4[4] so the engineer
// plugged into can0 can see how far this task got purely from CAN.
//   0 -> App_InitTask never started OR hung before any milestone
//   1 -> post-ErrorLatch::init
//   2 -> post-ErrorLatch::clear (only if AMS_HIL_CLEAR_ERROR_LATCH)
//   3 -> post-HAL_FDCAN_ConfigGlobalFilter
//   4 -> post-HAL_FDCAN_ActivateNotification
//   5 -> post-HAL_FDCAN_Start (returned non-OK)
...
// (pit-diag 0x6C4[5..7]) so the operator can distinguish init-stage
// HAL failures from later runtime issues.
...
void ams_app_init_task_run(void *argument)
{
    ...
    // VBAT-backed RTC_BKP_DR1 outlives long power-offs on the bench
    // (carrier has a coin cell + bulk caps), and the bench has no
    // external way to clear it. A stale latch from a previous session
    // would trap the bench in Error for the entire iteration.
    ...
    g_app_init_progress = 7u;   // reached self-exit (visible at pit-diag 0x6C4[4])
