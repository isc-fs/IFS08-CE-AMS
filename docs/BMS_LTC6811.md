...
`config::kLtcChainLength = 10` is the source of this count; any
...
`config::kAdg731ChannelMap[20]` encodes this as the lookup the
...
|---|---:|---|
| `kNtcBeta` | 3380 K | Murata NCP15XH103J datasheet |
| `kNtcR25` | 10 000 Ω | BMS_LITE BOM |
| `kNtcSeriesR` | 10 000 Ω | BMS_LITE BOM |
| `kNtcVrefMv` | 3000 | LTC6811 VREF2 nominal |
| `kNtcT0Kelvin` | 298.15 | 25 °C |

`BmsService::update_temperature` rejects readings outside
`[kNtcMinValidC, kNtcMaxValidC] = [-40, +150]` °C as
...
| MCU SPI1 | Baud | ~516 kHz (prescaler 256 on 132 MHz APB2) | `AMS.ioc` |
| MCU SPI1 | NSS | software, on PA4 | `AMS.ioc` |
| CS line | Wakeup pulse | 20 µs LOW, 30 µs HIGH | `ltc6820.cpp::kWakePulseUs` |
...
| Bus | Length | 10 ICs | `config::kLtcChainLength` |
...
lookup table (`kPec15Table`) generated by `constexpr` so the binary
...
3. `last_rx_tick[module]` only advances on a clean poll, so a
   sustained PEC-error storm naturally trips `kBmsStaleMs` and the
...
consecutive clean ICs. Mismatch with `kLtcChainLength` latches
`ErrorLatch`, opens all relays, posts `safety::kForceError`. See §9.
...
the frame with `Bus::write_chain_command(kCmdWRCFGA, ...)`.
...
2. `state.max_tempC <= kBalanceTempMax`

When permitted, per module the policy:

3. Marks any cell with `v > min_cell_mV + kBalanceDeltaMv` as a
   candidate.
4. Keeps at most `kBalanceMaxActive` candidates per module, picking
...
|---|---|---|
| Chain length ≠ `kLtcChainLength` on boot | `count_pec_valid_segments` after RDCFGA | `App_InitTask` → `ErrorLatch::set` + `Relays::open_all` + `kForceError`. ERROR latch persists across the follow-up watchdog reset. |
| PEC error on one IC, one register group | `decode_cell_voltage_group` / `decode_aux_voltage_group` returns false | drop slot, ++`g_ltc_pec_err_count[ic]`, continue. Sustained errors trip via freshness window (`kBmsStaleMs`). |
...
| Chain dropped to T_SLEEP | next ADCV broadcast lands while ICs are deaf → PEC fails | freshness window catches it within `kBmsStaleMs`. The temp poll's once-per-500-ms wakeup makes this extremely rare in practice. |
...
- BMS_LITE schematic (`pcbs/BMS_LITE/`):
  - `LTC_1.kicad_sch` / `LTC_2.kicad_sch` — per-LTC NTC routing,
    extracted into `kAdg731ChannelMap`
