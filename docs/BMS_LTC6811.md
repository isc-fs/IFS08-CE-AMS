...
| Open-wire detection (LTC6811 ADOW) | **live** (`config::CellOpenWireCheck = true`) — runs a two-pass ADOW scan on every voltage poll; software cell-mV plausibility still backs it for out-of-range opens | ADOW PUP=1/PUP=0 in `BmsPollTask::attempt_open_wire_poll` → `BmsService::update_open_wire` (per-IC 9/10 decode) → `ams::open_wire::detect_open_conductors` → `FaultReason::CellOpenWire` in any state, < 500 ms. Catches an open that still reads in-range, which the low-cell-V predicate misses. ⚠️ **The ADOW encoding + conversion timing are NOT yet hardware-validated** (bench down when this shipped) — validate on the chain (HIL #95) and watch for nuisance trips. |
