...
| **Charger** | The external HV charging station. Has **no bidirectional/handshake CAN protocol with the AMS** — it just one-way auto-emits the `0x101` "CHRG" frame (which the AMS reads as its charger precharge-proceed gate) and soft-starts its own output. |
...
| **TSMS** | Tractive System Master Switch (GPIO **PF9**) — a **held** master switch. Gates `Start → Precharge`; sustains `Run`/`Charge`. Its drop is a **non-latching** de-energise back to `Start` (#327) — not a fault — so the TS can be restarted from the cockpit unaided. |
...
| **Precharge** | Bringing the downstream bus capacitors up to pack voltage *through a resistor* before closing AIR+, to avoid a damaging inrush current. Used in **Car** mode; **Charger** mode skips the resistor (voltage-matched charger, no inrush) and closes AIR+ directly on the `0x101` proceed. The **precharge relay** is on PB7. |
...
| **Pit-diag** | The runtime-toggleable diagnostic CAN stream (`0x680..0x6C9`, 59 frames) carrying every cell, temp, FSM/fault detail, per-IC PEC count, and the FDCAN1 Bus-Off recovery count (`0x6C9`, #331) for bench debugging. |
| **DBC** | A CAN database file (`docs/dbc/ams.dbc`) describing every frame's signal layout, generated from the code-first CAN DSL (`Core/Inc/can/messages/*.def`) by [`tools/dbc_dump.cpp`](../../tools/dbc_dump.cpp). Consumed by MingoCAN. |
| **fault_reason** | A byte on pit-diag `0x6C0[6]` naming which predicate latched `Error` (enum 0..11; 12 = FsmError for FSM-driven errors like a precharge timeout). #276. |
...
| **Unity** | The C unit-test framework used for the host unit tests (`tests/unit/`). |
...
| **HIL** | Hardware-in-the-Loop — the real-hardware bench rig (MLC2 carrier + Pi Pico LTC emulator + CAN/GPIO fixtures). Acceptance plan: [issue #399](https://github.com/isc-fs/IFS08-CE-AMS/issues/399) (v1.6.2, supersedes #317). |
