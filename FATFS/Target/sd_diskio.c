...
ALIGN_32BYTES(static uint8_t scratch[BLOCKSIZE]) __attribute__((section(".sd_dma"))); // 32-Byte aligned in RAM_D1 for SDMMC IDMA & cache maintenance
#endif
/* Disk status */
...
#if (ENABLE_SD_DMA_CACHE_MAINTENANCE == 1)
        /* Clean D-Cache to flush CPU writes from cache into physical RAM before DMA starts */
